...
from chameleon_usage.math.timeseries import align_counts, time_weighted_resample
...
def test_align_fills_missing_timestamps_from_other_groups():
    ...
    result = align_counts(df, "ts", "val", ["group"]).collect().sort(["group", "ts"])
...
def test_align_preserves_original_timestamps():
    ...
    result = align_counts(df, "ts", "val", ["group"]).collect()
...
def test_align_with_multiple_group_columns():
    ...
    result = align_counts(df, "ts", "val", ["type", "source"]).collect()
...
def test_align_counts_returns_lazyframe():
    ...
    result = align_counts(df, "ts", "val", ["group"])
