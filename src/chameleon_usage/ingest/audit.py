...
def audit_to_intervals(
    ...
    Each row's ``audit_event_time`` (when the change happened in the source
    table) becomes ``start``; the next row's event time, per entity, becomes
    ``end``. DELETE rows only serve as end-markers and are dropped from output.
